...
// customers
router.get('/customers/new', common.restrict, (req, res) => {
  // only allow admin
  if(req.session.is_admin !== 'true'){
    res.render('error', {message: 'Access denied', helpers: req.handlebars, config: config});
    return;
...
router.post('/customer_insert', (req, res) => {
  ...
  db.customers.findOne({ 'customers_email': req.body.customers_email }, (err, customer) => {
    if(customer){
	  ...
	  // new customer
	  db.customers.insert(doc, (err, doc) => {
		// show the view
		if(err){
...
// update a customer
router.post('/customer_update', common.restrict, (req, res) => {
  const db = req.app.db;
  let bcrypt = req.bcrypt;
  //var is_admin = req.body.user_admin === 'on' ? 'true' : 'false';

  // get the customer we want to update
  db.customers.findOne({_id: common.getId(req.body.customer_id)}, (err, customer) => {
...
// delete customer
router.get('/customer/delete/:id', common.restrict, (req, res) => {
  // only allow admin
  if(req.session.is_admin !== 'true'){
    res.render('error', {message: 'Access denied', helpers: req.handlebars, config: config});
    return;
  ...
  // remove the customer
  if(req.session.is_admin === 'true'){
    db.customers.remove({_id: common.getId(req.params.id)}, {}, (err, numRemoved) => {
...
router.post('/customer_login', (req, res) => {
	...
	db.customers.findOne({'customers_email': req.body.customers_email}, (err, customer) => {
		if(customer){ // if customer exists with that email address
			...
			{$inc: { customers_nLogin: 1 }}, {multi: false}, (err, numReplaced) => {
				if(err){
			...
			// update last login
			db.customers.update({'customers_email': req.body.customers_email},
			{$set: { customers_lastLogin: dVal }}, {multi: false}, (err, numReplaced) => {
				if(err){
