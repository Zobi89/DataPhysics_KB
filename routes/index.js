...
router.get('/', common.restrict, function (req, res, next){
  ...
  common.dbQuery(db.kb, {kb_published: 'true'}, sortBy, config.settings.num_top_results, function (err, top_results){
    common.dbQuery(db.kb, {kb_published: 'true', kb_featured: 'true'}, sortBy, featuredCount, function (err, featured_results){
      res.render('index', {
        title: 'Data Physics Knowledge Base',
        user_page: true,
        homepage: true,
...
// customers
router.get('/customer/edit/:id', common.restrict, function (req, res){
  var db = req.app.db;
  db.customers.findOne({_id: common.getId(req.params.id)}, function (err, customer){
    // if the the current user is not
    // an admin we render an access denied message
    if(req.session.is_admin === 'false'){
      req.session.message = req.i18n.__('Access denied');
      req.session.message_type = 'danger';
      res.redirect(req.app_context + '/Customers/');
      return;
    }
...
// customers
router.get('/customers/new', common.restrict, function (req, res){
  // only allow admin
  if(req.session.is_admin !== 'true'){
    res.render('error', {message: 'Access denied', helpers: req.handlebars, config: config});
    return;
...
router.post('/customer_insert', function (req, res){
  var db = req.app.db;
  var url = require('url');
  var d = new Date();
  var dVal = d.valueOf();
  ...
  var doc = {
    ...
    customers_sysType: req.body.customers_sysType,
    customers_SN: req.body.customers_SN,
    customers_nLogin: 1,
    customers_lastLogin: dVal
  };

  // check for existing customer
  db.customers.findOne({'customers_email': req.body.customers_email}, function (err, customer){
    if(customer){
      // customer already exists with that email address
      console.error('Existing customer: ' + err);

	  req.session.message = req.i18n.__('Email is already registered');
	  req.session.message_type = 'danger';
	  res.redirect(req.app_context + '/welcome');
	}else{
	  // new customer
	  db.customers.insert(doc, function (err, doc){
		// show the view
		if(err){
		  console.error('Failed to insert customer: ' + err);
		  req.session.message = req.i18n.__('Failed to register customer');
		  req.session.message_type = 'danger';
		  res.redirect(req.app_context + '/');
		}else{
		  req.session.message = req.i18n.__('Welcome to Data Physics Knowledge Base');
		  req.session.message_type = 'success';
		  res.redirect(req.app_context + '/');
		}
	  });
...
// update a customer
router.post('/customer_update', common.restrict, function (req, res){
  var db = req.app.db;
  //var is_admin = req.body.user_admin === 'on' ? 'true' : 'false';

  // get the customer we want to update
  db.customers.findOne({_id: common.getId(req.body.customer_id)}, function (err, customer){
    // if the current user is not an admin we render an access denied message
    if(req.session.is_admin === 'false'){
      req.session.message = req.i18n.__('Access denied');
      req.session.message_type = 'danger';
      res.redirect(req.app_context);
    ...
    db.customers.update({_id: common.getId(req.body.customer_id)},
    {
      $set: {
        customers_name: req.body.customers_name,
        customers_company: req.body.customers_company,
        ...
        customers_sysType: req.body.customers_sysType,
        customers_SN: req.body.customers_SN,
        customers_nLogin: customer.customers_nLogin,
        customers_lastLogin: customer.customers_lastLogin
      }
    }, {multi: false}, function (err, numReplaced){
      if(err){
        console.error('Failed updating customer: ' + err);
        req.session.message = req.i18n.__('Failed to update customer');
        req.session.message_type = 'danger';
        res.redirect(req.app_context + '/customer/edit/' + req.body.customer_id);
      }else{
        // show the view
        req.session.message = req.i18n.__('Customer account updated');
        req.session.message_type = 'success';
        res.redirect(req.app_context + '/customer/edit/' + req.body.customer_id);
      }
    });
...
// delete customer
router.get('/customer/delete/:id', common.restrict, function (req, res){
  // only allow admin
  if(req.session.is_admin !== 'true'){
    res.render('error', {message: 'Access denied', helpers: req.handlebars, config: config});
    return;
  ...
  // remove the customer
  if(req.session.is_admin === 'true'){
    db.customers.remove({_id: common.getId(req.params.id)}, {}, function (err, numRemoved){
      req.session.message = req.i18n.__('Customer deleted');
      req.session.message_type = 'success';
      res.redirect(req.app_context + '/customers');
    });
  }else{
    req.session.message = req.i18n.__('Failed to delete customer');
    req.session.message_type = 'danger';
    res.redirect(req.app_context + '/customers');
  }
});
...
router.post('/customer_login', function (req, res){
	...
	// check for existing customer
	db.customers.findOne({'customers_email': req.body.customers_email}, function (err, customer){
		if(customer){ // if customer exists with that email address
			// increment login counter
			db.customers.update({'customers_email': req.body.customers_email},
			{$inc: { customers_nLogin: 1 }}, {multi: false}, function (err, numReplaced){
				if(err){
					console.error('Failed to increment login counter: ' + err);
				}
			}
			)
			// update last login
			db.customers.update({'customers_email': req.body.customers_email},
			{$set: { customers_lastLogin: dVal }}, {multi: false}, function (err, numReplaced){
				if(err){
					console.error('Failed to update last login: ' + err);
				}
			}
			)
			req.session.message = req.i18n.__('Welcome back');
			req.session.message_type = 'success';
			res.redirect(req.app_context + '/');
		}else{	// unknown email
			// show the view
			console.error('Login failed: ' + err);
			req.session.message = req.i18n.__('Unknown email address');
			req.session.message_type = 'danger';
			res.redirect(req.app_context + '/welcome');
		}
	});
